    ...
    for det in pred:
        if det is not None and len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                xyxy = scale_coords(img.shape[2:], xyxy, img0.shape).round()
...
while True:
    ...
    # Assuming card detections are provided as bounding box coordinates (x, y, w, h) for each detected card
    # Replace `card_results` with the output of your card detection model
...
# Draw the ratio on the frame
    text = "Face Width: {:.2f} mm".format(actual_face_width)
